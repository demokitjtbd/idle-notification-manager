...
// FIX: Corrected the function name in the import statement. The hyphen is not a valid character in an identifier.
...
const CreateFollowUp: React.FC<CreateFollowUpProps> = ({ onSave, onCancel, initialData }) => {
  ...
  const handleGenerateMessage = async (id: string) => {
    const details = formData.actionDetails as MessageActionDetails;
    const promptPair = details.prompts.find(p => p.id === id);
    if (!promptPair || !promptPair.prompt) return;

    setIsGenerating(id);
    try {
        const generatedMessage = await generateFollowUpMessage(promptPair.prompt);
        handlePromptDetailsChange(id, 'message', generatedMessage);
    } catch (error) {
                             ...
                             <div key={p.id} className="border bg-background/50 border-input rounded-lg">
                                ...
                                {!isCollapsed && (
                                    <div className="p-4 border-t border-input space-y-4 bg-background">
                                        ...
                                        <button type="button" onClick={() => handleGenerateMessage(p.id)} disabled={isGenerating === p.id} className="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-9 px-3 bg-secondary text-secondary-foreground hover:bg-secondary/80">
                                            {isGenerating === p.id ? 'Generating...' : `Generate Message with AI`}
                                        </button>
                                        <div>
                                            <label className={labelClasses}>Message*</label>
                                            <textarea value={p.message} onChange={e => handlePromptDetailsChange(p.id, 'message', e.target.value)} required rows={4} className={`${formElementClasses} h-auto`} placeholder="Type your message or generate one with AI."></textarea>
                                        </div>
